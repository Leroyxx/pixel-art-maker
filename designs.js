...
let originalGridWidth;
let originalGridHeight; //required for IDing grid additions,letiable will be incrememnted upon addition of height
let columnLeftPlus = 1; // times used addColumnLeft+1, this ensures a negative id for each new cell
let lineDownPlus = 0;
let lineUpPlus = 1; //times used addLineUp+1, this ensures a negative id for each new cell
let columnRightPlus = 1; //" " columnRight", " " " " " " " "
...
$(function toolTips() { //used code: github csasbach/tooltip.css
    ...
    $('#huh').mouseover(function() {
        ...
        let left = $(this).position().left + $(this).width() + 4;
        let top = $(this).position().top - 4;
...
$(function minimaxiWindowsFunctionality() {
    ...
    minimaxi.on("click", function(event) {
        let w = $(this).parent().next(".wrapper");
        let l = w.children(); //animation was ultimately based on a codepen by Matt Baxter (Expand/Collapse CSS Transition)
...
function placeSides() {
  ...
  if (528/windowWidth >= 0.48) {
    ...
    left.addClass("leftBe");
    right.addClass("rightBe");
  }
}
$(placeSides());
...
function changeSquareSizeLive() {
    let previousSquareSize = $("#input_size").val();
    let squareSize;
    let gridWidth = table.children().first().children().length;
    $(".drawable").css("width", previousSquareSize);
    $(".drawable").css("height", previousSquareSize);

    $("#input_size").on('change', function newSize() {
        squareSize = $(this).val();
        gridWidth = table.children().first().children().length;
...
function borderChangeFunctionality() {
    ...
    let borderChecked = $("#borderCheckbox").prop("checked");
    if (borderChecked) {
...
$(function gridMakingFunctionality() {
    ...
    function loadCode(pixelCode) {
        ...
        let indexS;
        for (let s = 3; s <= pixelCode.length - 1; s++) {
            if (s % 2 !== 0) {
                indexS = pixelCode[s] - 1;
            } else if (s % 2 === 0) {
                let cellColor = "#" + pixelCode[s];
...
function createSavePoint() {
    let gridWidthString = table.children().first().children().length;
    let gridWidth = parseInt(gridWidthString);
    let gridHeight = table.children().length;
    $("#containerSaveBox").hide();
    let pixelCode = [gridHeight, gridWidth];
    let pixelCodeCells = 1;
...
function saveSavePoint() {
    if (undoArray.length - 1 === savePointIndex) {
        ...
        for (let x = undoArray.length - 1; x >= savePointIndex + 2; x--) {
            undoArray.pop(x)
        }
        savePointIndex++;
    } else if (undoArray.length - 1 !== savePointIndex && undoType === "smartUndo") {
        let mirrorIndex = undoArray.length;
        for (let y = undoArray.length - 2; y >= savePointIndex - 1; y--) {
...
function undo() {
    ...
    let latestVersion = undoArray.slice(savePointIndex)[0];
    ...
    let indexS;
    for (let s = 2; s <= latestVersion.length - 1; s++) {

        if (s % 2 === 0) {
            indexS = latestVersion[s] - 1;
        } else if (s % 2 !== 0) {
            let cellColor = latestVersion[s];
            table.children().children().eq(indexS).css("background-color", cellColor);
        }
    }
...
let ctrlDown = false;
let color = "#00ced1"; //drawing color
let eraseColor = "#ffffff"; //erasing color
...
let toolIcons = {
    paintBucketIcon: {
        ...
        _2load: function(pixelCode) {
            ...
            for (let h = 1; h <= pixelCode[0]; h++) {
                this._place.append("<tr></tr>");
                for (let w = 1; w <= pixelCode[1]; w++) {
...
function displayEndResult(table) {
    ...
    let endCells = endResult.find(".drawable");
    let endRows = endResult.children().children();
    endCells.css("height", "");
    endCells.css("width", "");
...
$(function addToGridFunctionality() {
    ...
    removeLineDownJS.on("click", function(event) {
        removeLineDown();
    });
    removeLineUpJS.on("click", function(event) {
        removeLineUp();
    });
    removeColumnLeftJS.on("click", function(event) {
        removeColumnLeft();
    });
    removeColumnRightJS.on("click", function(event) {
        removeColumnRight();
    });
    ...
    function addLineDown() {
        let gridWidthString = table.children().first().children().last().attr("id").toString().split('_').pop();
        // gets last number of ID of last cell in first line
        let gridWidth = parseInt(gridWidthString);
        let gridWidthBeginID = table.children().first().children().first().attr("id").toString().split('_').pop();
        ...
        for (let w = gridWidthBeginID; w <= gridWidth; w++) {
    ...
    columnLeft.on("click", function(event) {
        let canvasWidth = table.children().first().children().length;
        let cellSize = squareSize = $("#input_size").val();
...
$(function drawingFunctionality() {
    let isDrawing = false; //drawing
    let rightMouseButton = false; //erasing
    let isFilling = false; //fill parts of grid with color
    ...
    function styleButtons() {
        if (tool === 1 && !rightMouseButton) {
            drawButton.css("border-bottom", `4px solid ${color}`);
            fillButton.css("border-bottom", "")
        } else if (tool === 2 && !rightMouseButton) {
            fillButton.css("border-bottom", `9px solid ${color}`);
            drawButton.css("border-bottom", "")
        } else if (tool === 1 && rightMouseButton) {
            drawButton.css("border-bottom", `4px solid ${eraseColor}`);
            fillButton.css("border-bottom", "")
        } else if (tool === 2 && rightMouseButton) {
            fillButton.css("border-bottom", `9px solid ${eraseColor}`);
            drawButton.css("border-bottom", "")
        }
    }
    ...
    function getCellW(cell) {
        let cellID = cell.attr("id").split('_');
    ...
    table.on('mousedown', '.drawable', function(event) {
        if (tool === 2 && event.which == 1 && !ctrlDown) {

            let clickedCellW = getCellW($(this));
            let clickedCellH = getCellH($(this));
            const clickedCellColor = $(this).css("background-color");
            if (rgb2hex(clickedCellColor) !== color) {
                fillToLimits(clickedCellH, clickedCellW)
            } else {
            ...
            function fillToLimits(cellH, cellW) {
                let cell = table.find(`#${cellH}_${cellW}`);
                let cellColor = cell.css("background-color");

                if (cellColor !== clickedCellColor) {} else {
        ...
        } else if (tool === 1 && event.which == 1 && !ctrlDown) {

            isDrawing = true;
            $(this).css("background-color", color);

        } else if (tool === 2 && event.which == 3 && !ctrlDown) {
            ...
            let clickedCellW = getCellW($(this));
            let clickedCellH = getCellH($(this));
            let clickedCellColor = $(this).css("background-color");
        ...
        } else if (tool === 1 && event.which == 3 && !ctrlDown) {
            rightMouseButton = true;
            $(this).css("background-color", eraseColor);
            ...
            toolIcons.penIcon._2load(toolIcons.penIcon._pixelCode);
            styleButtons();
        } else if (ctrlDown && event.which == 1) {
            color = $(this).css("background-color"); // color spooners
        } else if (ctrlDown && event.which ==3) {
            eraseColor = $(this).css("background-color")
        }
        displayEndResult(table);
    });
...
$(function saveFunctionality() {
    ...
    saveHTMLButton.on("click", function(event) {
        ...
        let endTableHTML = $(".end_result").html();
    ...
    saveCodeButton.on("click", function(event) {
        ...
        let gridHeight = table.children().length;
        $("#saveBox").children().remove();
        let pixelCode = [gridHeight, " " + gridWidth, " " + $("#input_size").val()];
        let pixelCodeCells = 1;
        //Function to convert hex format to a rgb color, credits to Mottie of JSFiddle
        function rgb2hex(rgb) {
            rgb = rgb.match(/^rgba?[\s+]?\([\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?/i);
